...
const Recognition_exam_intro = {
    ...
    stimulus: '<p>识别实验（6个刺激物）：试验都以 500 毫秒的固定时间开始，然后会有一个 500 毫秒的fixation，接下来是100毫秒的单个项目展示，' + ISI[0] + '毫秒的空白间隔后，会呈现一个六个项目的阵列。</p> ' +
        ...
        '<p>并根据数字键盘的 "4", "7", "9", "6", "3", "1" 进行选择响应，如果不记得或者不存在选择 "5"。</p>' +
        '<p>按"space"键继续。</p>',
    choices: [' '],
    post_trial_gap: 500,
}
...
const Recall_exam_intro = {
    ...
    stimulus: '<p>回忆实验（6个刺激物）：试验都以 500 毫秒的固定时间开始，然后会有一个 500 毫秒的fixation，接下来是 2500 毫秒的样本展示，' + ISI[0] + '毫秒的空白间隔后，会有一个目标刺激的提示。</p> ' +
        ...
        '<p>根据数字键盘的 "4", "7", "9", "6", "3", "1"  进行选择响应，如果不记得或者不存在选择"5"。</p>' +
        '<p>按"space"键继续。</p>',
    choices: [' '],
    post_trial_gap: 500,
}
...
//生成识别和回忆实验
// const recognition_shape_exam= generateRecognitionTrail(recognition_shape, dist_loc6,1,ISI[0],0,6,"shape")//36
const recognition_color_exam = generateRecognitionTrail(recognition_color, dist_loc6, 1, ISI[0], 0, 6, "color")
// const recall_shape_exam = generateRecallTrail(recall_shape, dist_loc6,1,ISI[0],0,6,"shape")//36
const recall_color_exam = generateRecallTrail(recall_color, dist_loc6, 1, ISI[0], 0, 6, "color")
...
`,
    on_finish: function (datas) {
        ...
        if (datas.response === 0) {
            data.get().localSave('csv', name + "_" + exam_type + "_" + ISI + "_6_" + type + "_Exam_pro_Memory-data.csv")
        }
    }
}
let stimArr = {
...
function f1(ISIs, TaskType, StudyAttribute) {
    ...
    if (TaskType === "Recognition") {
        if (StudyAttribute === "颜色") {
            ...
            exam = generateRecognitionTrail(stim, dist_loc6, 1, isi, 0, 6, "color")
            timeline.push(color_exam_intro)
            timeline.push(exam)
        } else if (StudyAttribute === "形状") {
            let stim = stimArr[TaskType]["shape"]
            exam = generateRecognitionTrail(stim, dist_loc6, 1, isi, 0, 6, "shape")
        ...
        if (StudyAttribute === "颜色") {
            let stim = stimArr[TaskType]["color"]
            exam = generateRecallTrail(stim, dist_loc6, 1, isi, 0, 6, "color")
            timeline.push(color_exam_intro)
            timeline.push(exam)
        } else if (StudyAttribute === "形状") {
            let stim = stimArr[TaskType]["shape"]
            exam = generateRecallTrail(stim, dist_loc6, 1, isi, 0, 6, "shape")
            timeline.push(shape_exam_intro)
            timeline.push(exam)
        }
    }
