...
const Recognition_introduce_exam = {
    ...
    stimulus: '<p>识别实验（两个刺激物）：每个试验都以 500 毫秒的固定时间开始，然后会有一个 500 毫秒的fixation，接下来是100毫秒的单个项目展示，' + ISI[0] + '毫秒的空白间隔后，会呈现一个两个项目的阵列。</p> ' +
        '<p>实验要求：记忆单个项目，并在多个项目阵列中快速找到，左边按数字键盘的 " 4 " ,右边按 " 6 " ,不存在或者不记得的选择 " 5 "。</p>' +
        '<p>按"space"键继续。</p>',
    choices: [' '],
    post_trial_gap: 500,
}
const Recall_introduce_exam = {
    ...
    stimulus: '<p>回忆实验（两个刺激物）：每个试验都以 500 毫秒的固定时间开始，然后会有一个 500 毫秒的fixation，接下来是 2500 毫秒的样本展示，' + ISI[0] + '毫秒的空白间隔后，会有一个目标刺激的提示。</p> ' +
        '<p>实验要求：记忆所呈现刺激物和其对应的位置，根据目标提示选择对应刺激的位置，左边按数字键盘的 " 4 " ,右边按 " 6 ",不存在或者不记得的选择 " 5 " 。</p>' +
        '<p>按"space"键继续。</p>',
    choices: [' '],
    post_trial_gap: 500,
}
...
//生成识别和回忆实验
// const recognition_shape_exam= generateRecognitionTrail(recognition_shape, dist_loc2,2,ISI[0],1,2,"shape")//18
const recognition_color_exam = generateRecognitionTrail(recognition_color, dist_loc2,2,ISI[0],1,2,"color")
// const recall_shape_exam = generateRecallTrail(recall_shape, dist_loc2,2,ISI[0],1,2,"shape")//18
const recall_color_exam = generateRecallTrail(recall_color, dist_loc2,2,ISI[0],1,2,"color")

// const recall_shape_exam_dist1 = generateRecallTrail(recall_shape, dist_loc2_1,2,ISI[0],1,2,"shape")//18
// const recall_color_exam_dist1 = generateRecallTrail(recall_color, dist_loc2_1,2,ISI[0],1,2,"color")
// const recall_shape_exam_dist2 = generateRecallTrail(recall_shape, dist_loc2_2,2,ISI[0],1,2,"shape")//18
// const recall_color_exam_dist2 = generateRecallTrail(recall_color, dist_loc2_2,2,ISI[0],1,2,"color")


const recognition_node_timeline = {
    timeline: [
        Recognition_introduce_exam,
        // shape_exam_intro,
...
`,
    on_finish: function (datas) {
        ...
        if (datas.response === 0) {
            data.get().localSave('csv', name + "_" + exam_type + "_" + ISI + "_2_shape and color_Exam_Memory-data.csv")
        }
    }
}
...
function f1(ISIs, TaskType, StudyAttribute) {
    ...
    if (TaskType === "Recognition") {
        if (StudyAttribute === "颜色") {
            ...
            exam = generateRecognitionTrail(stim, dist_loc2, 2, isi, 1, 2, "color")
            timeline.push(color_exam_intro)
            timeline.push(exam)
        } else if (StudyAttribute === "形状") {
            let stim = stimArr[TaskType]["shape"]
            exam = generateRecognitionTrail(stim, dist_loc2, 2, isi, 1, 2, "shape")
        ...
        if (StudyAttribute === "颜色") {
            let stim = stimArr[TaskType]["color"]
            exam = generateRecallTrail(stim, dist_loc2, 2, isi, 1, 2, "color")
            timeline.push(color_exam_intro)
            timeline.push(exam)
        } else if (StudyAttribute === "形状"){
            let stim = stimArr[TaskType]["shape"]
            exam = generateRecallTrail(stim, dist_loc2, 2, isi, 1, 2, "shape")
            timeline.push(shape_exam_intro)
            timeline.push(exam)
        } 
    }
