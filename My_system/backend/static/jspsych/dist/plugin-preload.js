var jsPsychPreload = (function (jspsych) {
  ...
  const info = {
      name: "preload",
      parameters: {
          /** Whether or not to automatically exam_preload any media files based on the timeline passed to jsPsych.run. */
          auto_preload: {
              type: jspsych.ParameterType.BOOL,
              pretty_name: "Auto-exam_preload",
              default: false,
          },
          /** A timeline of trials to automatically exam_preload. If one or more trial objects is provided in the timeline array, then the plugin will attempt to exam_preload the media files used in the trial(s). */
          trials: {
          ...
          /**
           * The maximum amount of time that the plugin should wait before stopping the exam_preload and either ending the trial
   ...
   * @see {@link https://www.jspsych.org/plugins/jspsych-preload/ exam_preload plugin documentation on jspsych.org}
   */
  class PreloadPlugin {
      constructor(jsPsych) {
      ...
      trial(display_element, trial) {
          ...
          // create list of media to exam_preload //
          var images = [];
          var audio = [];
          var video = [];
          ...
          if (in_safe_mode) {
              // don't exam_preload video if in safe mode (experiment is running via file protocol)
              video = [];
          }
          ...
          const on_success = () => {
              if (typeof timeout !== "undefined" && timeout === false) {
                  ...
                  // need to call cancel exam_preload function to clear global jsPsych preload_request list, even when they've all succeeded
