var jsPsychExtensionWebgazer = (function () {
    ...
    class WebGazerExtension {
        constructor(jsPsych) {
            ...
            let isBrowsingExternalPage = false;
            window.addEventListener('message', event => {
                // 检查消息来源是否是您的嵌套页面
                if (event.origin === 'http://localhost:63342') { // 替换为实际的域名
                    console.log(event.data)
                    if ('type' in event.data) {
                        // 用户退出了外部页面，设置 isBrowsingExternalPage 为 false
                        isBrowsingExternalPage = false;
                    }
                    else isBrowsingExternalPage = true;
                }
            });
            this.handleGazeDataUpdate = (gazeData, elapsedTime) => {
                //我的代码
                if (isBrowsingExternalPage === true) {
                        ...
                        if (event.origin === 'http://localhost:63342') { // 替换为实际的域名
                            if (gazeData !== null) {
                                ...
                                if (this.activeTrial) {
                                    ...
                                    let yOffset = Math.round(event.data.yOffset);
                                    if (!isNaN(yOffset)) {
                                        console.log(gazeData.y)
                                        d.y = this.round_predictions ? Math.round(gazeData.y + yOffset) : gazeData.y + Math.round(yOffset),
                                            console.log(d.y)
                                        this.currentTrialData.push(d);
                                    } else this.currentTrialData.push(d);
                                }
                                this.currentGaze = d;
