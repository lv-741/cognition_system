...
from static.pythonfiles.datafunc import preprocess_value, dfSave2
...
def datapro_dynamic(SIZE, PATH, PATH_SAVE):
    path = PATH
    path_save = PATH_SAVE
    ...
    for f in csv_files:
        ...
        # 将 NaN 值替换为字符串 "NaN"
        # df['response_target'] = df['response_target'].fillna("NaN")
        ...
        if (shape_index_0 is not None) & (color_index_0 is not None):
            ...
            df_save_shape = dfSave2(df_save_shape, new_df_shape, new_df_shape1)
            print(df_save_shape)
            # color
            df_save_color = pd.concat(
                [df_save_color, name_response_df], ignore_index=True
            )
            df_save_color = dfSave2(df_save_color, new_df_color, new_df_color1)
            print(df_save_color)
    ...
    df_save_shape.to_csv(
        path_save + "\\" + types[0] + "_" + SIZE + "_" + ".csv",
        index=False,
        encoding="utf-8",
    )
    df_save_color.to_csv(
        path_save + "\\" + types[1] + "_" + SIZE + "_" + ".csv",
        index=False,
        encoding="utf-8",
    )
