...
from static.pythonfiles.datafunc import dfSave, preprocess_value
...
def datapro_recall(ISI, SIZE, PATH, PATH_SAVE):
    # 地址
    path = PATH
    path_save = PATH_SAVE
    ...
    for f in csv_files:
        ...
        # 查找shape和color的起始行,recall需要-2
        if not df[processed_series.isin(arr_shape)].empty:
            shape_index_0 = df[processed_series.isin(arr_shape)].index[0] - 2
            shape_index_1 = df[processed_series.isin(arr_shape)].index[-1]
        else:
            shape_index_0 = None
            shape_index_1 = None
        # color不需要处理
        if not df["response_target"][df["response_target"].isin(arr_color)].empty:
            color_index_0 = (
                df["response_target"][df["response_target"].isin(arr_color)].index[0]
                - 2
            )
            color_index_1 = df["response_target"][
                df["response_target"].isin(arr_color)
            ].index[-1]
        else:
            color_index_0 = None
            color_index_1 = None
        ...
        if (shape_index_0 is not None) & (color_index_0 is not None):
            ...
            df_save_shape = dfSave(df_save_shape, new_df_shape, new_df_shape1)
            # color
            df_save_color = pd.concat(
                [df_save_color, name_response_df], ignore_index=True
            )
            df_save_color = dfSave(df_save_color, new_df_color, new_df_color1)
